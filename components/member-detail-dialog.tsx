...
import { User, Mail, Phone, Calendar, Hash, Building } from "lucide-react"
...
export function MemberDetailDialog({ open, onOpenChange, member }: MemberDetailDialogProps) {
  ...
  const getWorkTypeInfo = (workTypeId: string) => {
    if (workTypeId === "") {
      return { name: "미설정", bgcolor: "#f3f4f6", fontcolor: "#6b7280", startTime: "", endTime: "" }
    }
    if (workTypeId === "off") {
      return { name: "오프", bgcolor: "#e5e7eb", fontcolor: "#4b5563", startTime: "", endTime: "" }
    }

    const workType = workTypes.find((wt) => wt.id === workTypeId)
    if (!workType) {
      return { name: "알 수 없음", bgcolor: "#6b7280", fontcolor: "#ffffff", startTime: "", endTime: "" }
    }
                    ...
                    return (
                      <div key={day.key} className="text-center">
                        ...
                        >
                          ...
                          {workTypeInfo.startTime && workTypeInfo.endTime && (
                            <div className="text-xs opacity-80 mt-1">
                              {workTypeInfo.startTime.slice(0, 5)} - {workTypeInfo.endTime.slice(0, 5)}
                            </div>
                          )}
                        </div>
                      </div>
