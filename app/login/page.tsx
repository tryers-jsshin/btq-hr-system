...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await supabaseAuthStorage.login(formData.email, formData.password)
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-blue-50 p-3 rounded-lg">
                ...
                <p className="text-xs text-blue-600 mt-1">• 이메일: 구성원 등록 시 입력한 이메일 주소</p>
                <p className="text-xs text-blue-600">• 초기 비밀번호: 구성원 등록 시 입력한 전화번호</p>
              </div>
