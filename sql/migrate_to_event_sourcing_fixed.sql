...
BEGIN
  IF NOT EXISTS (
    ...
    ALTER TABLE work_mileage_transactions 
      ...
      CHECK (event_source IN ('attendance', 'leave', 'schedule', 'manual', 'legacy') OR event_source IS NULL);
  END IF;
END $$;
...
-- 새로운 업데이트 정책
DROP POLICY IF EXISTS "Allow mileage transaction updates" ON work_mileage_transactions;
...
-- 삭제 방지 정책
DROP POLICY IF EXISTS "No deletion of mileage transactions" ON work_mileage_transactions;
